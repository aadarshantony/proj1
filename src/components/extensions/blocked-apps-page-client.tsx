// src/components/extensions/blocked-apps-page-client.tsx
"use client";
...
export function BlockedAppsPageClient() {
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editItem
                  ? t("blockedApps.editTitle")
                  : t("blockedApps.addTitle")}
              </DialogTitle>
              <DialogDescription>
                {t("blockedApps.dialogDescription")}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="pattern">{t("blockedApps.patternLabel")}</Label>
                <Input
                  id="pattern"
                ...
                <p className="text-muted-foreground text-xs">
                  {t("blockedApps.patternHint")}
                </p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="name">{t("blockedApps.nameLabel")}</Label>
                <Input
                  id="name"
                  value={formData.name}
                  onChange={(e) =>
                  ...
                  placeholder={t("blockedApps.namePlaceholder")}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="reason">{t("blockedApps.reasonLabel")}</Label>
                <Textarea
                  id="reason"
                  ...
                  placeholder={t("blockedApps.reasonPlaceholder")}
                  rows={3}
                />
              </div>
            </div>
        ...
        <CardContent className="p-0">
          {items.length === 0 ? (
            <div className="py-10 text-center">
              <Ban className="text-muted-foreground/50 mx-auto h-12 w-12" />
              <p className="text-muted-foreground mt-4">
                {t("blockedApps.emptyTitle")}
              </p>
              <p className="text-muted-foreground mt-2 text-sm">
                {t("blockedApps.emptyDescription")}
              </p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t("blockedApps.tableHeaders.name")}</TableHead>
                  <TableHead>{t("blockedApps.tableHeaders.pattern")}</TableHead>
                  <TableHead>{t("blockedApps.tableHeaders.reason")}</TableHead>
                  <TableHead>{t("blockedApps.tableHeaders.status")}</TableHead>
                  <TableHead className="w-[100px]">
                    {t("blockedApps.tableHeaders.blockActive")}
                  </TableHead>
                ...
                {items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      ...
                      >
                        {item.enabled
                          ? t("blockedApps.statusBlocking")
                          : t("blockedApps.statusInactive")}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Switch
